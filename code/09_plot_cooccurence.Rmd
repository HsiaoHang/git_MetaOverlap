...
# log
@20250121
- Run the whole results, fig 3 shows even more convincing results on maximum length & length at infinity. So my manuscript can focus on length rather than k.

- Fig 2 I see Baltic Sea species (1 or 2) has alpha = NA. Check back to co-occurrence calculation to see what may be the problem. --> both Baltic Sea species has so high overlap subareas among pairs, so alpha is at the ceiling for each year and pair. 

- Check Jhih-Sheng's cpue.pair: cpue for x and y is only at intersect area? Because the name says intersect.--> It is correct: we used cpue_length_subarea, not cpue_intersect_length_subarea.

Next:

- Double check whole script to confirm the results
- Do Fig S2, Fig S3


# Changes for revision:
- Replace SO by alpha
- Remove Baltic Sea species
- Remove fecundity
- Put maximum body length as main result
- 
...
# load dd.co & house keeping
```{r}

dd.co <- read.csv("/Users/hsiaohang.tao/Dropbox/Academic/Ecoinformatics/a1_Meta_Overlap/analysis_meta_overlap/data/co.dd.csv")
...
write_excel_csv(trait.table,"/Users/hsiaohang.tao/Dropbox/Academic/Ecoinformatics/a1_Meta_Overlap/analysis_meta_overlap/output/figure_final/trait.table.csv")


```


# remove Baltic populations
```{r}
...
#0 add alpha from <dd.co> to <cpue.pair> as a new column
...
@20250122 change to maximum length
output 6 x 9
```{r}
...
# 9.0 ratio of positive relationships
...
#9.1 ratio of sig over all relationships
...
#8 [Fig S1] plot other life history traits
...
p0 <- plot.life.history('length.max','Maximum body size')
...
cpue.pair.ext <- cpue.pair %>% 
  ...
  # calculate mean cpue of x and y
  mutate(mean_cpue_ext = mean(cpue_length_yearqua_x,cpue_length_yearqua_y, trim = 0)) %>% 
  #@20250122 add alpha_mle
  dplyr::select(region, species, year, area_overlap, 
                mean_cpue_ext, alpha_mle) %>% 
  rename('so_ext' = 'area_overlap') %>% 
  rename(alpha_ext = alpha_mle)
...
cpue.pair.nea <- cpue.pair %>% 
  ...
  mutate(mean_alpha_nea = mean(alpha_mle))
...
so.dd <- so.dd %>% 
  ...
  #add the same for alpha
  mutate(diff_alpha = alpha_ext - mean_alpha_nea) %>% 
  mutate(percent_diff_alpha = (diff_alpha / mean_alpha_nea)*100)

# bind two [cpue.pair.ext] , [cpue.pair.nea] to create [so.dd.long] as long form
cpue.pair.ext <- cpue.pair.ext %>% 
  mutate(so_group = 'ext') %>% 
  rename('so' = 'so_ext') %>% 
  rename(alpha = alpha_ext)

cpue.pair.nea <- cpue.pair.nea %>% 
  mutate(so_group = 'adj') %>% 
  rename('so' ='mean_so_nea') %>% 
  rename(alpha = mean_alpha_nea)
...
# sort by maximum length, by adding growth.coefficient from [trait] to [so.dd.long] 
so.dd.long <- inner_join(so.dd.long, trait.name.k, 
           by = c('region','common_name'))

so.dd.long$common_name <-
  fct_reorder(so.dd.long$common_name,                                 desc(so.dd.long$length.max))
...
@20250122 replace growth.coefficient by length.max
```{r}
...
##5 [fig S3 (a)] plot alpha~maximum length relationship by pair
output 3 x 7
```{r}
